...
if uploaded_file is not None:
    ...
    num_pages = fitz.open('upload_folder/' + uploaded_file.name).page_count
    images = convert_pdf_to_images('upload_folder/' + uploaded_file.name, num_pages)
    ...
    if st.button('Process'):
        if opt_concat_or_not:
            ...
            os.system(command)
            os.system(command2)
        os.remove('upload_folder/' + uploaded_file.name)